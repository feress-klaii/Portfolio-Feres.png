...
import "./SingularityHologram.css";

// Same accretion-disk shader/scene as the reference, but: sized to a
// small container instead of the full window, locked permanently to
// the "Stable Singularity / Topology: Nominal" state (the state-cycle
// and its 10s interval are removed), and non-interactive — it's a
// decorative hologram beside the Explore list, not something meant
...
function SingularityHologram() {
  ...
  useEffect(() => {
    ...
    const camDist = NOMINAL.camDist + 15;
    const autoRotateSpeed = NOMINAL.rotate;
    let manualAngle = 0;
    let camYCurrent = NOMINAL.camY;

    // drag-to-rotate: orbit angle + camera height both respond to
    // pointer drag, eased toward the target like the rest of the
    // site's motion. Autoplay drift continues underneath the drag.
    const drag = { targetAngle: 0, targetY: NOMINAL.camY, active: false, lastX: 0, lastY: 0 };
    ...
    function onPointerMove(e) {
      ...
      drag.lastX = e.clientX;
      drag.lastY = e.clientY;
      drag.targetAngle -= dx * 0.006;
      drag.targetY = Math.min(60, Math.max(6, drag.targetY + dy * 0.15));
    }
    ...
    function animate() {
      ...
      manualAngle += autoRotateSpeed * 0.0026;
      const easedAngle = manualAngle + drag.targetAngle;
      camYCurrent += (drag.targetY - camYCurrent) * 0.08;

      camera.position.x = Math.cos(easedAngle) * camDist;
      camera.position.z = Math.sin(easedAngle) * camDist;
